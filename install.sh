...
get_local_ddns_go_version() {
    ...
    version_output=$("${DDNS_GO_BIN_PATH}" -version 2>&1) || version_output=$("${DDNS_GO_BIN_PATH}" version 2>&1)
    
    # 如果上述命令失败，尝试从二进制文件中提取版本信息
    if [[ $? -ne 0 ]]; then
        version_output=$(strings "${DDNS_GO_BIN_PATH}" | grep -E 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -n 1)
    fi
